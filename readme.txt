...
= 3.19.4 =
* Updated the MyStyle dashboard to link the various counts to their source admin
  pages.
* Updated the design title on the design profile page to include who the design
  was created by.
* Now setting the default styling on the design tag cloud.

= 3.19.3 =
* Added standard WordPress pagination to Design Tags and Design Collections 
  index pages.

= 3.19.2 =
* Fixed issue with removing products from existing designs.
* Changed Design Tag Index page title.
...
* Now allowing the user to be passed through the customizer (to fix a bug where
  the user was occasionally dropped).
* Now adding a "mystyle-design-product-sold-out" class to the body tag on the
  product page if the product is sold out.

= 3.18.8 =
* Design collections now show 24 terms instead of 10.
* Added a new "mystyle-design-tag-index" CSS class to the main div on the 
  design tag index page.
* Added a new "mystyle-design-author" CSS class to the "Designed By:" div on the
  design tag index page and the design collection index page.
* Namespaced the "design-collection-index" CSS class to 
  "mystyle-design-collection-index" (on the design collection index page).
* My Designs page now returns a "No designs yet. Create one now!" message if 
  there aren't any designs.
* Now using the "mystyle-my-designs" class instead of the 
  "mystyle-design-profile" on the My Designs page.
* The product title is now appended to the design title on the Design Profile
  page. 
* If a product id is passed into the customize page, it is now always used 
  instead of the one from the design that is being reloaded.
