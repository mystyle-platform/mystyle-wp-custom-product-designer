...
abstract class MyStyle_DesignManager extends \MyStyle_EntityManager {
	...
	$design_id, WP_User $user = null, MyStyle_Session $session = null
	) {
		global $wpdb;
	...
	public static function delete(MyStyle_Design $design) {
		...
		$ret = $wpdb->delete(
				MyStyle_Design::get_table_name(), array(MyStyle_Design::get_primary_key() => $design->get_design_id()), array('%d')
		);
	...
	$current_design_id, WP_User $user = null
	) {
		global $wpdb;
	...
	$per_page = 250, $page_number = 1, WP_User $user = null
	) {
		global $wpdb;
