...
class MyStyle_Design_Profile_Page {
	...
	private function init_index_request() {
		...
		$designs = MyStyle_DesignManager::get_designs(
						$this->pager->get_items_per_page(), $this->pager->get_current_page_number(), $this->user
		);
		$this->pager->set_items($designs);

		//total items
		$this->pager->set_total_item_count(
				MyStyle_DesignManager::get_total_design_count(), $this->user
		);
	...
	static function show_add_to_cart_button() {
		return MyStyle_Options::is_option_enabled(
						MYSTYLE_OPTIONS_NAME, 'design_profile_page_show_add_to_cart', true // default to true
		);
	}
