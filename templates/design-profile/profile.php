...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_design_nonce'])) {
	...
	if (wp_verify_nonce($nonce, 'mystyle_delete_design_nonce')) {
		// Perform the design deletion
		$design->set_access(MyStyle_Access::ACCESS_RESTRICTED);
		MyStyle_DesignManager::persist($design);
		$delete_message = "Design deleted successfully!";
	} else {
		// Display error message
		$delete_message = "Error deleting design. Please try again.";
	}
}
?>
...
<div id="mystyle-design-profile-wrapper" class="woocommerce">
	...
	<?php if (isset($delete_message)) : ?>
		<div class="design-delete-message">
			<h3 style="color: <?php echo isset($nonce) ? 'green' : 'red'; ?>;"><?php echo esc_html($delete_message); ?></h3>
		</div>
	<?php endif; ?>
	...
	<div class="product_description">
		...
		<?php if ($author) : ?>
			<div class="linked_user">
				Designer: <a href="<?php echo esc_url(MyStyle_Author_Designs_Page::get_author_url($author)); ?>" title="<?php echo ((is_string($author)) ? 'Anonymous User' : esc_attr($author->display_name)); ?> Designs"><?php echo ((is_string($author)) ? 'Anonymous User' : esc_html($author->display_name)); ?></a>
			</div>
		<?php endif; ?>
		<?php
		if (
			...
			|| (current_user_can('administrator')) //administrator
				...
				current_user_can('administrator') || (MyStyle_DesignManager::is_user_design_owner($current_user->ID, $design->get_design_id()))
			) {
				if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_design_nonce'])) {
					$nonce = sanitize_text_field($_POST['delete_design_nonce']);
